...
int binarySearch(int size, char arr[][255], char *toFind, int *count) {
	int low = 0;
	int mid;
	int high = size - 1;
	*count = 0;
...
void spell_check(char * testname, char * dictname, int printOn){
	...
	while(token != NULL)
	{
		...
		int index = binarySearch(size, dictionary, lowerCase, &count);
		...
		else
		{
			...
			else
			{
				if(input > 0)
				{
					fprintf(outname, "%s", minDistWords[input - 1]);
				}
		...
		{
			fprintf(outname, "%c", symbol[0]);
		}
		
		else if(strcmp(symbol, "\0") != 0 && *(symbol + 1) == ' ')
		{
			fprintf(outname, "%c", symbol[1]);
		}
